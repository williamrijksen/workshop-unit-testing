...
describe('PersonService', () => {
  ...
  const mockErrorResponse = {status: 400, statusText: 'Bad Request', message: 'Oopsie'}; // This can be used as a mock error in following tests.
  ...
  it('should log and throw an error if the second attempt also fails', () => {
    ...
    service.getPersons().subscribe((persons) => {}, (err) => {
     // Deze nog doen.
    });
    let request = httpMock.expectOne(service.apiUrl + 'persons');
    request.flush(errorCode, mockErrorResponse);
